...
FL_ErrorStatus FL_ATIM_Init(ATIM_Type *TIMx, FL_ATIM_InitTypeDef *TIM_InitStruct)
{
    ...
    /* 时钟总线使能配置 */
    FL_CMU_SetATIMClockSource(TIM_InitStruct->clockSource); //选择时钟源
    FL_CMU_EnableGroup4BusClock(FL_CMU_GROUP4_BUSCLK_ATIM); //高级定时器ATIM 总线时钟使能，高有效
    FL_CMU_EnableGroup3OperationClock(FL_CMU_GROUP3_OPCLK_ATIM); //高级定时器ATIM 工作时钟使能，高有效
    /* 设置重复计数值 */
